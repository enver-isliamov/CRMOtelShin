...
- **Исправление дублирования данных:** При импорте данных из Google Sheets, если у записей отсутствует `id` (например, колонка `id` не создана в таблице), теперь генерируется детерминированный хэш на основе уникальных полей (Договор, Имя, Телефон). Это предотвращает создание дубликатов при повторных синхронизациях.
